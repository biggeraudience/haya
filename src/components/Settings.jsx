...
// Assuming ProductNavbar and ProductFooter are needed in the standalone Settings page
// If this component is only used within FeedSection, you might not need these imports here.
// However, keeping them based on the user's provided code.
import { useSettings } from "../context/SettingsContext"; // Import settings context

const Settings = () => {
  // Access global settings and updater from the context
  const { settings, updateSettings } = useSettings();

  // Handler for updating individual setting keys
  const handleChange = (key, value) => {
    updateSettings(key, value);
  };

  // Handler specifically for product layout change (calls the generic handleChange)
  const handleLayoutChange = (newLayout) => {
    handleChange('productLayout', newLayout); // Update layout in the context
  };

  // Define SVGs for themes - using inline SVG for simplicity and self-containment
  // Note: These SVGs are placeholders. You might want to replace them with actual icons.
  const themeIcons = {
    default: (
  ...
  // Define SVGs for product layouts - using inline SVG for simplicity and self-containment
  // Note: These SVGs are placeholders. You might want to replace them with actual icons.
  const layoutIcons = {
    grid: (
      <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    swipe: (
      <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        {/* Corrected path data with escaped forward slashes */}
  ...
  return (
    <>
      {/* Including Navbar and Footer based on the provided code.
          If this component is only used within FeedSection, these might be redundant. */}
      ...
      <div className="settings-page">
        <div className="main-box">
          <div className="inner-container">
                ...
                {["default", "light", "dark", "pastel", "greyscale"].map((theme) => (
                  <div className="option" key={theme}>
                    <label className="checkbox-label" htmlFor={theme}>
                      ...
                      />
                      {/* Render the corresponding theme icon */}
                      {themeIcons[theme]}
                      {/* Display theme name with first letter capitalized */}
                ...
                {[
                  ...
                  <div className="option" key={language}>
                    <label className="checkbox-label" htmlFor={language}>
                      ...
                      />
                      {/* Display language name with first letter capitalized */}
                ...
                {["small", "medium", "large"].map((size) => (
                  <div className="option" key={size}>
                    <label className="checkbox-label" htmlFor={size}>
                      ...
                      />
                      {/* Display size name with first letter capitalized */}
                  ...
                  <div className="option" key={layout}>
                    <label className="checkbox-label" htmlFor={layout}>
                      <input
                        ...
                        onChange={() => handleLayoutChange(layout)} // Use the specific handler
                      />
                      {/* Render the corresponding layout icon */}
                      {layoutIcons[layout]}
                      {/* Display layout name with first letter capitalized */}
                ...
                {["creditCard", "paypal", "googlePay"].map((method) => (
                  <div className="option" key={method}>
                    <label className="checkbox-label" htmlFor={method}>
                      ...
                      />
                      {/* Display method name with first letter capitalized */}
            ...
            <div className="setting-tab">
              ...
              <div className="overflow-container">
                <div className="option">
                  {/* Input for minimum product limit */}
                  <input
                    type="number"
                    placeholder="Min"
                    value={settings.productLimits?.min || ''} // Use optional chaining and default to empty string
                    onChange={(e) =>
                      handleChange("productLimits", {
                        ...settings.productLimits,
                        min: Number(e.target.value), // Convert input value to number
                      })
                    }
                  />
                  {/* Input for maximum product limit */}
                  <input
                    type="number"
                    placeholder="Max"
                    value={settings.productLimits?.max || ''} // Use optional chaining and default to empty string
                    onChange={(e) =>
                      handleChange("productLimits", {
                        ...settings.productLimits,
                        max: Number(e.target.value), // Convert input value to number
                      })
                    }
                  />
                </div>
            ...
            {/* Settings Navigation - Save button */}
      ...
      {/* Including Footer based on the provided code.
          If this component is only used within FeedSection, this might be redundant. */}
