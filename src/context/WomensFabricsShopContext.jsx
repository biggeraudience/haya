...
export const WomensFabricsShopContext = createContext();

const WomensFabricsShopProvider = ({ children }) => {
  const [products, setProducts] = useState([]);
  const [filterProducts, setFilterProducts] = useState([]);

  // Filter states for women's fabrics
  ...
  // Fetch all women's fabrics products using public route
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get('/products/public?category=womensfabrics');
        setProducts(response.data || []);
        setFilterProducts(response.data || []);
      } catch (err) {
        console.error('Error fetching women fabrics:', err);
      }
    };
    fetchProducts();
  }, []);
  ...
  return (
    <WomensFabricsShopContext.Provider
      value={{
        products,
        filterProducts,
        selectedSubcategory,
...
export const useWomensFabricsShop = () => useContext(WomensFabricsShopContext);

export default WomensFabricsShopProvider;
